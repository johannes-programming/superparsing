...
test: beautiful
	conda run -n base python make/env.py test_superparsing --python=3.11 --recreate >/dev/null;
	conda run -n test_superparsing pip install -e . >/dev/null;
	conda run -n test_superparsing python run_tests.py;
	conda run -n test_superparsing pip install mypy >/dev/null;
	conda run -n test_superparsing python -m mypy --strict .;
	conda run -n test_superparsing python -m mypy --strict -p superparsing;
