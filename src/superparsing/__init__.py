...
def parse(
    ...
    prog:str, 
    help_opts:list[str],
    version_opts:list[str],
    version:str,
):
    parser = argparse.ArgumentParser(prog=prog, add_help=False)
    if help_opts:
        parser.add_argument(*help_opts, action='help')
    if version_opts:
        parser.add_argument(*version_opts, action='version', version=version)
    parser.add_argument("words", nargs="*")
    space = parser.parse_args(args)
    print(json.dumps(space.words))